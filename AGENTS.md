...
- `src/app/studenti/` + `src/lib/studenti/akce.ts` — modul studentů (F1 onboarding): seznam (verca/meira/admin) se sloupcem stavu plánu, založení BEZ plánu (meira/admin), detail s plánem, tlačítko „poslat přihlašovací odkaz". Uvítací e-mail přes Resend (`src/lib/email/`). Úklid testovacích dat: `scripts/smaz-studenta.ts`.
- **Návrh a potvrzení plánu (use case „telefonát Veroniky", migrace 0002):** student se zakládá bez plánu; Veronika (verca/admin) na detailu studenta zadá délky v měsících (ACC 9–36, PCC/celkem 18–60; komplet obě — `generujPlanZDelek` v `src/lib/plan/generator.ts`, rozsahy `ROZSAHY_DELEK`) a pošle návrh (`src/lib/plan/navrh-akce.ts`). Studentovi přijde e-mail s přesnými texty Aleše + jednorázový token-odkaz na veřejnou `/plan/potvrzeni` (výjimka v proxy; potvrzuje až tlačítko na stránce kvůli e-mailovým skenerům, po úspěchu redirect na `/plan/potvrzeni/hotovo`). Potvrdit jde i z profilu (`potvrdMujPlan`). Potvrzením `plan_potvrzen_at` → závazný e-mail se seznamem termínů. Nový návrh jde poslat, jen dokud nic neodevzdal (jinak editace plánů R21); přepíše položky, zneplatní starý odkaz, resetuje potvrzení. **Do potvrzení nelze nahrávat (guard v `overPolozkuProUpload`) a neběží připomínková kaskáda** (filtr v denní kontrole). Stávající/migrovaní studenti mají plán automaticky potvrzený (UPDATE v migraci 0002; import R36 nastavuje `plan_potvrzen_at` rovnou).
